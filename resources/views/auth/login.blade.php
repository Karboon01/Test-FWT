...
@section('content')
    <form method="POST" action="{{ route('login') }}" class="flex flex-col justify-center">
        ...
        <div class="mb-5">
            ...
            <input id="email" :value="old('email')" type="email" name="email" required autofocus
        ...
        <div class="flex items-center justify-end mt-4 gap-5">
            <a class="text-sm text-neutral-400 hover:text-white"
                href="{{ route('register') }}">
                Ещё не зарегистрированны?
            </a>

            <button type="submit" class="bg-neutral-600 px-4 py-2 rounded border-2 border-neutral-400 hover:bg-neutral-500 duration-150 ease-in-out">Войти</button>
        </div>
    </form>
@endsection
